...
namespace rdmaio
{
  ...
  namespace qp
  {
    ...
    public:
      ...
      using CreateDCTRes_t = Result<std::pair<ibv_dct *, std::string>>;
                                       ...
                                       int dc_key)
      {
        if (cq == nullptr)
        {
          return Err(std::make_pair<ibv_dct *, std::string>(nullptr,
                                                            "CQ passed in as null"));
        }
        ...
        struct ibv_dct_init_attr dctattr;
        ...
        auto dct = ibv_create_dct(nic->get_ctx(), &dctattr);
        if (dct == nullptr)
        {
